...
public class GapListElement extends GapList {
    ...
    int getLargestGap(int yetLargest) {
        //TODO Überprüfen, ob es so funktioniert
    ...
    public int getLargestGap() {
        ...
        return this.getPayload().getLength() > this.next.getLargestGap() ? this.getPayload().getLength() : this.next.getLargestGap();
    }
    ...
    public GapList insert(Gap gap) {
        if (this.getPayload().touches(gap) || this.getPayload().overlaps(gap)) {
            ...
            GapListElement newGapListElement = new GapListElement(newUnionedGap, this.next);

            if (!this.next.isEmpty() && (newGapListElement.next.getPayload().touches(newUnionedGap) || newGapListElement.next.getPayload().overlaps(newUnionedGap))) {
                return newGapListElement.next.insert(newUnionedGap);
            }

            return newGapListElement;
        } else if (!this.next.isEmpty() && gap.precedes(this.next.getPayload())) {
            this.next = new GapListElement(gap, this.next);
            return this;
        } else {
